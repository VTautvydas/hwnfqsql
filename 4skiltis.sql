...
SELECT Books.title AS Books, GROUP_CONCAT(TRIM(Authors.name) SEPARATOR ', ') AS Authors FROM Books INNER JOIN Authors ON Books.bookId = Authors.bookId GROUP BY Books.bookid;
